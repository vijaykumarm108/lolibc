...
/// \class numeric_limits LoUlimits.h std.h
/// \brief Defines numeric limits for a type.
///
template <typename T> 
struct numeric_limits {
    /// Returns the minimum value for type T.
    static inline T minV (void)		{ return (T(0)); }
    /// Returns the minimum value for type T.
    static inline T maxV (void)		{ return (T(0)); }
...
struct numeric_limits<T*> {
    static inline T* minV (void)	{ return (NULL); }
    static inline T* maxV (void)	{ return (reinterpret_cast<T*>(UINTPTR_MAX)); }
...
struct numeric_limits<type> {					\
    static inline type minV (void)	{ return (minVal); }	\
    static inline type maxV (void)	{ return (maxVal); }	\
...
#ifdef _WIN64
_NUMERIC_LIMITS (intptr_t,		0,			SIZE_MAX,	true,	true,	true);
_NUMERIC_LIMITS (size_t,		0,			SIZE_MAX,	false,	true,	true);
#endif
...
#define NUMERIC_LIMITS(type, minVal, maxVal, bSigned, bInteger, bIntegral)	\
	namespace lo { namespace std { _NUMERIC_LIMITS (type, minVal, maxVal, bSigned, bInteger, bIntegral); } } 
