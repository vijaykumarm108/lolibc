...
namespace lo
{
	...
	template<typename T, bool isClass = __is_class(T)> class newvectorbase
	{
	};

	template<typename T>
	class newvectorbase<T,true> : public reflink
	{
	public:
		newvectorbase()	{}
	...
	template<typename T>
	class newvectorbase<T,false> : public reflink
	{
	public:
		newvectorbase()	{}
	...
	template<typename T>
	class newvector : public newvectorbase<T>
	{
	public:
		...
		/** Removes all elements from the vector, calling their respective destructors, leaving the container with a size of 0. The capacity of the vector does not change.*/
		void					clear();
		/** Returns a const_reverse_iterator pointing to the last element in the container (i.e., its reverse beginning). */
		const_reverse_iterator	crbegin() const;
		/** Returns a const_reverse_iterator pointing to the element right before the first element in the container (which is considered its reverse end). */
		...
		//newvector& operator= (initializer_list<value_type> il);
		...
		/** rbegin points to the element right before the one that would be pointed to by member end. */
		const_reverse_iterator	rbegin() const;
		/** rend() points to the theoretical element that precedes the one that would be pointed to by member begin(). */
		reverse_iterator		rend();
		/** rend() points to the theoretical element that precedes the one that would be pointed to by member begin(). */
		const_reverse_iterator	rend() const;
		/** Requests that the vector capacity be at least enough to contain n elements. This only causes allocation in order to grow not to shrink. */
		void					reserve (size_type n);
		/** Resizes the container to contain \p n elements, inserting default constructed values.  */
		void					resize( size_type n );
		/** Resizes the container to contain \p n elements and copies the new empty values to val */
		void					resize( size_type n, const value_type & val );
		/** Requests the container to reduce its capacity to fit its size. It is designed to save space, however size is not guarenteed to equal capacity after this call. */
		void					shrink_to_fit();
		/** Returns the number of used elements in the vector */
		size_type				size() const;
		/** swaps the contents of the vector with \p x. */
