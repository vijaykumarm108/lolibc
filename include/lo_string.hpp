...
///		is more convenient. Because std does not implement locales,
...
/// do anything else, you are hard-coding yourself into a locale! So stop it!
///
...
public:
    ...
    typedef ::lo::std::reverse_iterator<iterator>		reverse_iterator;
    typedef ::lo::std::reverse_iterator<const_iterator>	const_reverse_iterator;
...
public:
	...
	__declspec( property( get=get_Length ) ) int Length;	int get_Length() { return (int)this->length(); }

	// Public System::String Methods
	string Format(const char *format, void *arg0);
	inline int LastIndexOf( char value, int offset = string::npos )			{ return (int)this->find_last_of(value,offset); }
	inline int LastIndexOf( const char *value, int offset=string::npos )	{ return (int)this->find_last_of(value,offset); }
	inline string Substring(int offset, size_type length = string::npos)	{ return this->substr((uoff_t)offset, (size_type)length ); }
	string ToLower();
	string ToUpper();
...
//----------------------------------------------------------------------
// Class wstring is only for the cases when we need to conver to and from widechar strings
class wstring
{
public:
	inline wstring() : buffer(nullptr) {}
	wstring(const char *_init) { init(_init); }
	wstring(int size)	{ buffer = reinterpret_cast<wchar_t *>(malloc(size*sizeof(wchar_t))); *buffer = 0; }
	~wstring();
