...
namespace lo { namespace std {
    ...
    #define VectorSize(v)	(sizeof(v) / ::lo::std::size_of_elements(1, v))
#endif

/// Expands into a ptr,size expression for the given static vector; useful as link arguments.
#define VectorBlock(v)	(v)+0, (int)VectorSize(v)	// +0 makes it work under gcc 2.95
...
inline T1 minV (const T1& a, const T2& b)
{
    return (a < b ? a : b);
}
...
template <typename T1, typename T2>
inline T1 maxV (const T1& a, const T2& b)
{
    return (b < a ? a : b);
}
...
inline uint64_t bswap_64 (uint64_t v)
{
    ...
    if (!__builtin_constant_p(v)) asm ("bswap %0":"+r"(v)); else
#endif
	v = (uint64_t(bswap_32((uint32_t)v)) << 32) | uint64_t(bswap_32(v >> 32));
    return (v);
}
#endif
...
#endif
    { r = (uint32_t)FirstBit(r,r); if (r >= (uint32_t)BitsInType(r)-1) r = uint32_t(-1); }
    return (1<<(1+r));
}
