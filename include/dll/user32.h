/*
Known previously as winuser.h, this has been relabeled to user32.h
*/
#pragma once
#include <dll.h>
#include "gdi32.h"
...
//__drv_when((uFlags&(EWX_POWEROFF|EWX_SHUTDOWN|EWX_FORCE))!=0, __drv_preferredFunction("InitiateSystemShutdownEx", "Legacy API. Rearchitect to avoid Reboot"))
WINUSERAPI BOOL WINAPI ExitWindowsEx( __in UINT uFlags, __in DWORD dwReason);
WINUSERAPI BOOL WINAPI SwapMouseButton( __in BOOL fSwap);
...
WINUSERAPI BOOL WINAPI IsWindowEnabled( __in HWND hWnd);
WINUSERAPI HACCEL WINAPI LoadAcceleratorsA( __in_opt HINSTANCE hInstance, __in LPCSTR lpTableName);
WINUSERAPI HACCEL WINAPI LoadAcceleratorsW( __in_opt HINSTANCE hInstance, __in LPCWSTR lpTableName);
#ifdef UNICODE
...
WINUSERAPI HACCEL WINAPI CreateAcceleratorTableA( __in_ecount(cAccel) LPACCEL paccel, __in int cAccel);
WINUSERAPI HACCEL WINAPI CreateAcceleratorTableW( __in_ecount(cAccel) LPACCEL paccel, __in int cAccel);
#ifdef UNICODE
...
WINUSERAPI BOOL WINAPI InsertMenuItemA( __in HMENU hmenu, __in UINT item, __in BOOL fByPosition, __in LPCMENUITEMINFOA lpmi);
WINUSERAPI
BOOL
WINAPI
InsertMenuItemW(
 ...
 * Set debug level
 */
WINUSERAPI VOID WINAPI SetDebugErrorLevel(__in DWORD dwLevel);
...
WINUSERAPI VOID WINAPI SetLastErrorEx( __in DWORD dwErrCode, __in DWORD dwType);
...
WINUSERAPI HMONITOR WINAPI MonitorFromRect( __in LPCRECT lprc, __in DWORD dwFlags);
WINUSERAPI HMONITOR WINAPI MonitorFromWindow( __in HWND hwnd, __in DWORD dwFlags);
...
WINUSERAPI VOID WINAPI NotifyWinEvent( __in DWORD event, __in HWND  hwnd, __in LONG  idObject, __in LONG  idChild);
