/**
All references originating from GDI32.DLL
Target OS: Desktop Windows XP, Vista, 7 & 8, Server 2003, 2008, 2012
*/
#pragma once
...
typedef struct _DRAWPATRECT {
        POINT ptPosition;
        POINT ptSize;
        unsigned short wStyle;
        unsigned short wPattern;
...
WINGDIAPI int  WINAPI DescribePixelFormat(  __in HDC hdc, __in int iPixelFormat, __in UINT nBytes, __out_bcount_opt(nBytes) LPPIXELFORMATDESCRIPTOR ppfd);
...
WINSPOOLAPI int WINAPI DeviceCapabilitiesA( __in LPCSTR pDevice, __in_opt LPCSTR pPort, __in WORD fwCapability, LPSTR pOutput, __in_opt CONST DEVMODEA *pDevMode );
WINSPOOLAPI int WINAPI DeviceCapabilitiesW( __in LPCWSTR pDevice, __in_opt LPCWSTR pPort, __in WORD fwCapability, LPWSTR pOutput, __in_opt CONST DEVMODEW *pDevMode );
#ifdef UNICODE
...
#endif // !UNICODE
WINGDIAPI BOOL  WINAPI GetCharWidthFloatA( __in HDC hdc, __in UINT iFirst, __in UINT iLast, __out_ecount(iLast + 1 - iFirst) PFLOAT lpBuffer);
WINGDIAPI BOOL  WINAPI GetCharWidthFloatW( __in HDC hdc, __in UINT iFirst, __in UINT iLast, __out_ecount(iLast + 1 - iFirst) PFLOAT lpBuffer);
#ifdef UNICODE
...
WINGDIAPI BOOL  WINAPI GetCharABCWidthsA( __in HDC hdc,
                                            __in UINT wFirst,
                                            __in UINT wLast,
                                            __out_ecount(wLast - wFirst + 1) LPABC lpABC);
WINGDIAPI BOOL  WINAPI GetCharABCWidthsW( __in HDC hdc,
                                            __in UINT wFirst,
                                            __in UINT wLast,
                                            __out_ecount(wLast - wFirst + 1) LPABC lpABC);
#ifdef UNICODE
...
WINGDIAPI BOOL  WINAPI GetCharABCWidthsFloatA( __in HDC hdc, __in UINT iFirst, __in UINT iLast, __out_ecount(iLast + 1 - iFirst) LPABCFLOAT lpABC);
WINGDIAPI BOOL  WINAPI GetCharABCWidthsFloatW( __in HDC hdc, __in UINT iFirst, __in UINT iLast, __out_ecount(iLast + 1 - iFirst) LPABCFLOAT lpABC);
#ifdef UNICODE
...
WINGDIAPI int   WINAPI GetDIBits( __in HDC hdc, __in HBITMAP hbm, __in UINT start, __in UINT cLines,  __out_opt LPVOID lpvBits, LPBITMAPINFO lpbmi, __in UINT usage);  // SAL actual size of lpbmi is computed from structure elements
...
WINGDIAPI UINT WINAPI GetOutlineTextMetricsA( __in HDC hdc,
                                                __in UINT cjCopy,
                                                __out_bcount_opt(cjCopy) LPOUTLINETEXTMETRICA potm);
WINGDIAPI UINT WINAPI GetOutlineTextMetricsW( __in HDC hdc,
                                                __in UINT cjCopy,
...
WINGDIAPI BOOL WINAPI GetTextExtentPointA( __in HDC hdc, __in_ecount(c) LPCSTR lpString, __in int c, __out LPSIZE lpsz );
WINGDIAPI BOOL WINAPI GetTextExtentPointW( __in HDC hdc, __in_ecount(c) LPCWSTR lpString, __in int c, __out LPSIZE lpsz );
#ifdef UNICODE
...
WINGDIAPI BOOL WINAPI GetTextExtentPoint32A( __in HDC hdc, __in_ecount(c) LPCSTR lpString, __in int c, __out LPSIZE psizl );
WINGDIAPI BOOL WINAPI GetTextExtentPoint32W( __in HDC hdc, __in_ecount(c) LPCWSTR lpString, __in int c, __out LPSIZE psizl );
#ifdef UNICODE
...
WINGDIAPI BOOL WINAPI GetTextExtentExPointA( __in HDC hdc, __in_ecount(cchString) LPCSTR lpszString, __in int cchString, __in int nMaxExtent,
    __out_opt LPINT lpnFit, __out_ecount_part_opt (cchString, *lpnFit) LPINT lpnDx, __out LPSIZE lpSize );
WINGDIAPI BOOL WINAPI GetTextExtentExPointW( __in HDC hdc, __in_ecount(cchString) LPCWSTR lpszString, __in int cchString, __in int nMaxExtent,
    __out_opt LPINT lpnFit, __out_ecount_part_opt (cchString, *lpnFit) LPINT lpnDx, __out LPSIZE lpSize );
#ifdef UNICODE
...
WINGDIAPI BOOL  WINAPI SetPixelFormat(__in HDC hdc, __in int format, __in CONST PIXELFORMATDESCRIPTOR * ppfd);
 ...
 WINGDIAPI BOOL  WINAPI StretchBlt(__in HDC hdcDest, __in int xDest, __in int yDest, __in int wDest, __in int hDest, __in_opt HDC hdcSrc, __in int xSrc, __in int ySrc, __in int wSrc, __in int hSrc, __in DWORD rop);
...
WINGDIAPI UINT WINAPI GetDIBColorTable( __in HDC  hdc, __in UINT iStart, __in UINT cEntries, RGBQUAD *prgbq);
...
#endif // !UNICODE
 WINGDIAPI BOOL  WINAPI ExtTextOutA( __in HDC hdc, __in int x, __in int y, __in UINT options, __in_opt CONST RECT * lprect, __in_ecount_opt(c) LPCSTR lpString, __in UINT c, __in_ecount_opt(c) CONST INT * lpDx);
 WINGDIAPI BOOL  WINAPI ExtTextOutW( __in HDC hdc, __in int x, __in int y, __in UINT options, __in_opt CONST RECT * lprect, __in_ecount_opt(c) LPCWSTR lpString, __in UINT c, __in_ecount_opt(c) CONST INT * lpDx);
#ifdef UNICODE
