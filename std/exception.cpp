...
// Reads the exception from stream \p is. JCS
void bad_alloc::read (istream& is)
{
    lo::std::exception::read (is);
...
{
    return (lo::std::exception::stream_size() + stream_size_of(m_nBytesRequested));
}
...
{
    return (exception::stream_size() + stream_size_of(m_Errno) + stream_size_of(m_Operation));
}
